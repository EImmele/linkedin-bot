...
const cismIntro = `💡 Como parte da minha preparação para o exame de certificação CISM (Certified Information Security Manager) da ISACA, estou compartilhando reflexões práticas que conectam minha experiência no mercado com o conhecimento adquirido nesta jornada.`;
...
Como é feita a tradução dos riscos de TI para a linguagem de negócios na sua organização?
...
A equipe de Segurança da Informação da sua empresa é vista como uma parceira estratégica ou como o departamento do "não"?
...
Quando ocorre um vazamento crítico de dados, a sua equipe tem um plano de resposta testado ou o caos toma conta das primeiras 2 horas?
...
async function main() {
    for (const key of Object.keys(updatedPosts)) {
        ...
        console.log(`Updated Word doc with CISM Intro: ${filePath}`);
    }
}
